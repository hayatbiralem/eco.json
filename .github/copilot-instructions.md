...
- **fromToPositionIndexed.json**: Pre-indexed transition data (optimized for O(1) position lookups)
- **scores.json**: Position evaluation scores in pawns (1.00 = 100 centipawns, generated by eco.json.tooling)
...
- Published files for download: fromToPositionIndexed.json, scores.json
...
- ✅ Downloads `fromToPositionIndexed.json` from eco.json GitHub (no longer bundled)
- ✅ Downloads `scores.json` from eco.json GitHub (no longer bundled)
- ✅ Complete migration - zero bundled data files from eco.json repo

## Migration History

**Completed migrations in fensterchess:**

1. **fromToPositionIndexed.json** (3.7 MB eliminated from bundle):
   - Was: Bundled locally in `data/fromToPositionIndexed.json`
   - Now: Downloads from `https://raw.githubusercontent.com/JeffML/eco.json/master/fromToPositionIndexed.json`
   - Functions: `getFromTosForFen.js`, `getMasterGamesByPosition.js`
   - Pattern: Module-level cache, fetch on cold start

2. **scores.json** (1.2 MB eliminated from bundle):
   - Was: Bundled locally in `data/scores.json`
   - Now: Downloads from `https://raw.githubusercontent.com/JeffML/eco.json/master/scores.json`
   - Function: `scoresForFens.js`
   - Pattern: Module-level cache, fetch on cold start
