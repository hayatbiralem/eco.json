...
## TODO: fensterchess Migration

**Opportunities to simplify fensterchess using eco.json methods:**

1. **Eliminate serverless function** `netlify/functions/getFromTosForFen.js`:
   - Replace with: `import { getFromTos } from '@chess-openings/eco.json'`
   - Benefits: No serverless function maintenance, no API auth needed, already returns Opening objects

2. **Remove bundled data file** `data/fromToPositionIndexed.json`:
   - The npm package downloads this on-demand from GitHub
   - Benefits: Smaller deployment bundle, automatic updates when data changes

3. **Simplify enrichment code**:
   - Current: Fetch FEN arrays, then map to Opening objects
   - New: `getFromTos()` returns Opening objects directly
   - Remove manual FEN → Opening mapping in `findOpening()`

4. **Reduce dependencies**:
   - Can remove position book building (use `getPositionBook()` from package)
   - Already using same types as eco.json package
